...
\begin{table}
  \begin{tabular}{lllll}
    ...
    Method  & Year &  Reference & Cited $^*$\\ 
		...
		\bottomrule
\end{tabular}
\caption[Related work]{Overview of automated haplogrouping tools for human mtDNA, $^*$number of citations as of June, 2017 in Google Scholar\footnote{\url{https://scholar.google.com/}} \# tool not published in a peer reviewed journal} 
\label{table:tools}
\end{table}
...
about the data quality and can check the corresponding samples. The varying coverage in the exome data becomes and problems with the VCF file in the 1000G phase 3 data become visible (see Table 1
...
for each fragment compare the phylogenetic distance $\gamma_f(i)$ of the resulting haplogroup and calculate the total score $\sum_{n=1}^{length(f)} |{\gamma_f(P_{f_i} - R_{f_i})| > 5}$ where all samples with distances in the phylogenetic Tree surpassing five nodes are emitted as potentially recombinant.
\end{algorithm}
...
\begin{figure}[!ht]
    ...
    \caption[Run time comparison of HaploGrep versions]{Run time comparison of different MicroArrays. HG2 represents the HaploGrep 2 algorithm, Multimap the optimized hash based approach} 
    \label{hg:multimap2}
\end{figure}
\end{enumerate}
