...
\begin{algorithm}
\caption{Querying profiles with Phylotree database}
\label{list:pseudo}

Iterate over all entries in Phylotree $P$ with length $ 1...l_{P}= |P|$ and all entries in query profile $Q$ with length $1...l_{Q} = |Q|$.

For all polymorphisms in $P_i$ and in Range $R$ use weight $w$ according algorithm \ref{alg:haplo2}:

a) ${expected}$ = polymorphism in $P$

b) ${found}$ = polymorphisms found in $Q$ and $R$, 

c) ${remaining}$ = polymorphisms in $Q$ and $R$ and not in $P$

d) ${out}$ = polymorphism not in $Q$ and not in $R$
