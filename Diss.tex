...
Enabling Scalable Data Analysis of Mitochondrial DNA in the Era of High-Throughput Data Generation
\\[40pt]}
...
Mitochondriale DNA (mtDNA) spielt in einer Vielzahl von Disziplinen eine wichtige Bedeutung: unter anderem um genetische Ereignisse in der Evolution zu rekonstruieren, in der Forensik zum identifizieren von Personen oder sterblichen \"Uberresten, sowie in klinischen Studien und Forschung aber auch in Therapie-Ans\"atzen. Durch die steigende Anzahl von genetischen Daten, welche durch neue Hochdurchsatz-Methoden mit steigendem Datenvolumen in immer gr\"o{\ss}eren Studienkollektiven massiv erh\"oht werden, steigt auch die Fehlerwahrscheinlichkeit und erfordert neue Methoden zur Qualit\"atskontrolle. Diese vorliegende Arbeit besch\"aftigt sich mit Algorithmen und Methoden, welche implementiert als Softwaresystem zu einem Workflow kombiniert werden. Aspekte von Sequenzalignment, hin zum parallelen Mapping, Detektion von Mutationen und Annotation der mtDNA werden in dieser Arbeit vorgestellt, und neue Ans\"atze zur Qualit\"atskontrollen beschrieben. Mit Hilfe phylogenetischer Cluster, Haplogruppen genannt, wird ein Verfahren zur Detektion von Kontaminationen vorgestellt, das der Ver\"offentlichung von falschen und irref\"uhrenden Ergebnissen vorbeugen kann, und in Hinblick auf die ansteigenden Datenmengen eine neuartige effiziente Methode darstellt.
...
\cleardoublepage
\include{chapterHaploGrep}
\cleardoublepage
\include{chapterAlignment}
\cleardoublepage
\include{chaptermtDNA-Server}
\cleardoublepage
\include{chapterContamination}
\cleardoublepage
\include{outlook}
\cleardoublepage
